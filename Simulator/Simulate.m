function [tsim, Xsim, alpha, calibre, T, M] = Simulate(R, S)
...
%       - M     : flexion values at query points and query times
    ...
    T = zeros(length(S.tquer), length(S.xquer));
    M = zeros(length(S.tquer), length(S.xquer));
    ...
    % does a valid step.
        ...
        if isempty(flag) || strcmp(flag, 'init')
            ...
            if(tquer_i<=length(S.tquer) & t>=S.tquer(tquer_i))
                ...
                z = R.getAll_z();
                l = R.getAll_l();
                [T(tquer_i,:), M(tquer_i,:)] = Flexion(m', l', z', S.xquer, a(1), S.xquer(end));
